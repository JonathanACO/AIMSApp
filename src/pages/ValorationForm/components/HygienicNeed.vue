...
const riesgoCaidas = computed(() => {
  ...
  if (puntuacionTotalRiesgoCaidas.value >= 2 && puntuacionTotalRiesgoCaidas.value < 4) return "Mediano";
  return "Bajo";
});

function handleUpdateFactoresDeRiesgoSeleccionados(factors: {
  factorDeRiesgo: string;
  puntuacion: number;
}[]) {
...
<template>
  <div class="flex justify-between items-center bg-sky-100 px-4 py-1 my-4">
    ...
    <img class="h-8 w-8" src="../../../assets/icons/hygienic.png" alt="Icono de seccion"/>
  </div>
  ...
  <div class="mx-4 my-6">
    ...
    <PatientStateTable
        class="w-full mb-3"
        @update:puntuacion-riesgo-caidas="handleUpdateFactoresDeRiesgoSeleccionados"
    />
