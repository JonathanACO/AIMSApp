...
import { formatDateToNumeric } from "../../utils/formatDates";
...
const formattedName = formatNurseName(nurse!.name);
...
onMounted(getPatientAppointments);
</script>
<template>
  <IonPage>
    ...
    <div v-else class="px-10 py-10 font-sans">
      <div>
      ...
      >
        ...
        >
          <h3 class="text-xl py-4 bg-blue-50">
            Fecha de la consulta: {{ formatDateToNumeric(appointment.createdAt)
            }}<br />Enfermero(a):
            {{ formattedName }}
          </h3>
          ...
          <div class="flex justify-between mt-4 px-2 text-lg">
            <span v-if="currentIndex > 0">
              {{
                formatDateToNumeric(appointments[currentIndex - 1].createdAt)
              }}
            </span>
